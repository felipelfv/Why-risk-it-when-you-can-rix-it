...
libraries those packages rely on. We define these and related computing terms,
which may be unfamiliar to readers without a computer science background, in
...
calculates and incorporates results (e.g., Table 1) and figures (e.g., Figure 1), 
...
| **System-level libraries** | Shared operating-system software that R packages depend on (e.g., BLAS/LAPACK, C++ backends); not installed by `install.packages()`. |
| **Workflow orchestration** | Automating the order of analysis steps and re-running only those affected by a change (e.g., the {targets} package). |
...
| **Functional programming language** | A language whose functions depend only on their inputs; Nix is one, but {rix} writes it for you. |
