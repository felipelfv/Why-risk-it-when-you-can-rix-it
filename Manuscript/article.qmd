...
reproducibility (i.e., the ability to precisely reproduce the results of a 
study or studies based on provided data, code, materials, and 
software/hardware) remains limited [@hardwicke2020]. 
...
At the lower end, researchers may share only their manuscript; 
further along, they might provide partial code, full analysis scripts, or 
publicly accessible datasets; at the highest level, they document a fully 
specified computational environment that allows others to recreate 
identical results (from raw data to final manuscript output) with minimal 
friction. Open-science initiatives have 
made considerable progress along this continuum. Journal incentives such as 
...
This distinction matters because data and code are 
never self-sufficient. They depend on a hierarchy of software 
components—collectively known as dependencies—including the programming 
language version, the packages used, and the system libraries those 
packages require in order to function correctly. When these dependencies 
differ from those used in the original analysis, code may fail, 
behave differently across machines and time, or yield conflicting 
...
which rely on complex codebases, versioned dependencies, and intricate 
software configurations [@luijken_etall_2024; @siepe_etall_2024]. 

Researchers have identified several factors that support reproducibility,
including literate programming, version control, dependency management,
virtual machines, containerization, and thorough documentation
[@peikert_brandmaier_2021; @ziemann_etall_2023; @wiebels_moreau_2021].
Most of these practices presuppose a stable computational environment;
virtual machines and containerization attempt to provide one.
Therefore, we use *computational environment* to refer to this complete software
context—the programming language version, package versions, system libraries,
and operating system required for an analysis to run successfully
[@rodrigues_2023; @rodrigues_baumann_2026_polyglot]. 
*Computational environment reproducibility*, then, is the ability to 
reconstruct this entire software stack on any machine and at any future time, 
such that executing the same code yields the same numerical results.
...
Arguably, these challenges persist, in part, because researchers must navigate
a fragmented landscape of solutions, each addressing only part of the problem.
Package-level managers such as {renv} [@ushey_2024] and
...
they assume the software stack required to run each step is already stable.
Virtual machines and containerization tools, such as Docker, offer more
comprehensive approaches by bundling the full environment—operating system,
system libraries, interpreter versions, and packages—into a single shareable
unit. Virtual machines, however, can be large and slow, making them impractical
to share [@wiebels_moreau_2021]. Containerization is more lightweight, and 
R-focused implementations like Rocker [@boettiger_2015; 
@boettiger_eddelbuettel_2017] have made it more accessible. 
Yet building containers still requires familiarity
...
see @rodrigues_baumann_2026_polyglot. Thus, researchers are left with options 
that are either accessible but incomplete or powerful but demanding of more 
technical expertise.

In response to that, we introduce Nix [@dolstra_etall_2004], a functional 
package manager designed to make software installation deterministic, and {rix}
[@rodrigues_baumann_2025], an R interface that allows researchers to use Nix
without needing deep knowledge of its underlying language or infrastructure.
Our focus is specifically on computational environment reproducibility as the
foundation upon which other reproducibility practices depend. We do not
prescribe a particular workflow orchestration system or analytic structure.
Instead, we aim to show how Nix and {rix} can establish a stable, 
cross-platform environment within which any
...
tools (e.g., Make, {targets} or {rixpress}) or embedded as code chunks 
in literate programming documents (e.g., `.rmd` or `.qmd`)—can be executed 
reliably.
...
Positron. Similarly, we use `apaquarto` because of its relevance to
psychological journals, but Nix is agnostic to the manuscript template employed.
Later in the article, we highlight additional considerations and briefly comment 
on {rixpress} [@rixpress], which extends Nix-based reproducibility to workflows 
spanning multiple languages. 
Throughout, our emphasis remains squarely on the reproducibility of 
...
the computational details one might encounter in practice. While readers can 
follow the tutorial without engaging with the specifics of the simulation, 
we include it to anchor the discussion in something more tangible. The code 
implementing this simulation follows a conventional format: multiple
component files, each handling a distinct analytical phase. [^2]: 
data generation (`Simulation_Scripts/01_data_generation.R`), statistical 
model specification (`Simulation_Scripts/02_models.R`), simulation execution 
(`Simulation_Scripts/03_run_simulation.R`), performance metric calculation 
(`Simulation_Scripts/04_performance_metrics.R`), and results visualization 
(`Simulation_Scripts/05_plots.R`).
...
Notably, the simulation study depends on several R packages 
(see Appendix A). Thus, let's pretend a researcher whats to reproduce our 
simulation and results reported in the article. 
...
language itself introduces version dependencies. Code written for a recent
version of R may use syntax or functions unavailable in earlier versions
(e.g., native pipe operator `|>`, introduced in R 4.1). More subtly, R's random 
number generator has changed across major versions, meaning
that identical code with identical seeds can produce different random sequences
depending on the R version [@ottoboni_stark_2018]. 
For simulation studies where reproducibility of
...
Importantly, and we will come back to this later on, although Nix is capable of 
replacing tools like Docker for isolation or {renv}
...
capabilities lie beyond the present focus on environment reproducibility. 
...
felipelfv@Felipes-MacBook-Pro Why-risk-it-when-you-can-rix-it %
  nix-shell --expr "$(curl -sl https://raw.githubusercontent.com/ropensci/rix/
    main/inst/extdata/default.nix)"
```
...
manuscript using `apaquarto`, we also need associated system and LaTeX 
dependencies. For our project, we specify the environment as follows 
(@lst-rix-env-manuscript; on the GitHub repository as a file 
named `generate_env.R`[^3]):

[^3]: In the GitHub repository, the argument `r_pkgs` contains one extra 
    package: {rixpress}. This is related to the fact we also attached 
    extra information about {rixpress} for the interested reader as part of the
...
Note that not all packages listed are required for the simulation and results 
themselves—packages like {knitr} and {svglite} are required because of `.qmd`.
For the simulation and results alone, specifying `date` (see below) and 
the relevant `r_pkgs` (e.g., {rvinecopulib}, {doParallel},
{doRNG}, {marginaleffects}, etc) would suffice. Appendix B details the reasons for
each package. For now, we focus on clarifying the `rix()` arguments.
...
After defining the environment with `rix()`, the environment specification 
must be generated by executing the corresponding `.R` script. From the terminal, 
navigate to the study directory and execute 
(@lst-generate-env):
...
Generate the environment specification \normalsize
:::

Alternatively, the same script can be executed from within R: 
`source("generate_env.R")` or `rix::rix()`. This step produces the 
`default.nix` file in the project directory, 
which serves as the formal, machine-readable specification of the computational 
environment. Importantly, `rix()` automatically invokes `rix_init()`, which 
creates a project-specific `.Rprofile` file that prevents package library 
conflicts and disables `install.packages()` to maintain environment integrity. 
There may be exemptions to this, which we comment briefly in the next section.
...
In other words, the code in the aforementioned separate .R files inside the 
folder Simulation_Scripts are included as code chunks
...
For the interested reader, in the GitHub repository we included a folder in 
which the entire simulation and manuscript are produced using {rixpress}.
...
Projects with existing Docker-based infrastructure may wish to combine Nix
...
[@rodrigues_baumann_2025]. This strategy may be particularly relevant, 
for example, for projects requiring deployment to cloud computing platforms or
