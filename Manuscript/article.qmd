...
libraries those packages rely on. When these differ from the originals, code may
...
set.seed(123)                                 # fix the starting position once
conditions <- expand.grid(n = c(50, 100), 
                          gamma2 = c(0, 0.3)) # 4 conditions
nsim    <- 1000                               # repetitions per condition
states  <- matrix(list(), nrow(conditions), nsim)
results <- matrix(NA_real_, nrow(conditions), nsim)

for (cond in seq_len(nrow(conditions))) {     # for loop's for purposes of illustration
  for (r in seq_len(nsim)) {
    states[[cond, r]] <- .Random.seed         # state at the start of cell (cond, r)
    x <- rnorm(conditions$n[cond])            # data generation advances the state
...
#| lst-cap: "Reproducible parallel streams with {doRNG}, storing the per-iteration
#| state so any single repetition within a condition can be reproduced"
#| eval: false
#| echo: true

library(doRNG)

set.seed(123)                                # makes the streams below reproducible
rng_states <- vector("list", nrow(designs))  # one slot per condition

for (k in seq_len(nrow(designs))) {          # for loop's for purposes of illustration
  res <- foreach(
    i = seq_len(nsim),
    .packages = c("marginaleffects", "rvinecopulib")
  ) %dorng% {
  ...
  rng_states[[k]] <- attr(res, "rng")      # per-repetition stream states
