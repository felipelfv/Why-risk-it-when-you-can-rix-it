...
[@peng_2011], where a study's position depends on what its authors make
available, from publication alone at the low end to code and data that are
linked and directly executable at the high end. Reaching that executable end is
often treated as *full* reproducibility 
...
Arguably, these challenges may persist for multiple reasons, one highly relevant
being that even researchers aware of these demands must navigate a fragmented
landscape of solutions, each addressing only part of the problem.
...
[^1]: We leave workflow orchestration aside for two reasons. First, many tools
    ...
    although formal orchestration adds value, a study whose scripts are well
    ...
    reproducibility spectrum. Thus, a stable computational environment is what
    the other practices ultimately rest on, and it is what we focus on here. For
    readers interested in articles with a larger focus on the workflow, we
    suggest @peikert_etall_2021 and @vanlissa_etall_2021.
