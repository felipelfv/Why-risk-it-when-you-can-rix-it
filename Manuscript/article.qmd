...
author-note:
  disclosures:
    ...
    data-sharing: "Data and materials from this study can be accessed at https://github.com/felipelfv/Why-risk-it-when-you-can-rix-it.git."
format:
  apaquarto-pdf:
    document-mode: man
    keep-tex: true
...
[@peikert_brandmaier_2021; @epskamp_2019; @wiebels_moreau_2021]. Assuming the
data and code are error-free, reproducibility depends on a hierarchy of software
...
Positron. At the end of the article, we also briefly introduce {rixpress}
[@rixpress], a workflow orchestration tool that builds on Nix to coordinate
pipelines across R, Python, and Julia.
...
entire software stack—R itself, R packages, and all system libraries—at the time
of this writing. This eliminates the system-dependency problems that tools like
{renv} cannot address [@rodrigues_baumann_2025]. This architecture also ensures
stability over time. Empirical work has shown strong rebuildability and
reproducibility rates for historical nixpkgs snapshots
...
if R was already installed (@lst-install-rix-cran):
...
automatically load the Nix environment when the project directory is opened[^3].
...
[^3]: **`direnv`** is a lightweight utility that integrates with the user’s
...
running `rix()` in an R console (@lst-rix-env-manuscript) or from the terminal
using `Rscript` (@lst-rscript-generate):
...
[^4]: Please note that activating an RStudio instance via Nix does **not**
...
[^6]: Although we use *apaquarto* in this example, many alternative manuscript
...
scripts that depend on one another. For example, `03_run_simulation.R` begins by
...
document rendering in Step I), the simulation study may be reproduced within the
Nix shell (@lst-run-simulation) as follows:
...
specified in `default.nix`. In the same way, we could proceed with
`04_performance_metrics.R` which loads the simulation results (in
`Simulation_Scripts/sim_results.rds`) and calculates the performance metrics,
and `05_plots.R` uses those saved metrics (in
`Simulation_Scripts/performance_summary.rds`) to create the plots. Note,
however, that this approach relies on manually running scripts in sequence. It
ensures a reproducible environment but does not formalize the workflow itself;
dependencies between scripts remain implicit in the code rather than explicitly
declared.
...
repository includes a branch demonstrating {rixpress} for this entire project.
