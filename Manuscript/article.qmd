...
is reproducibility (i.e., the ability to exactly reproduce the results of a study based 
...
sufficient [@epskamp_2019; @wiebels_moreau_2021; @ziemann_etall_2023]. Even
error-free code depends on a hierarchy of software components, including the
programming language version, the packages used in the analysis, and the system
libraries those packages rely on. We define these and related terms,
which may be unfamiliar to some readers, in
@tbl-glossary. When these differ from the originals, code may
...
this ideal. Indeed, @siepe_etall_2024 report that, among simulation studies 
published in psychology in 2021 and 2022, nearly two-thirds provide 
...
::: {.callout-note}
JG:How many papers did Siepe look at? 
:::

::: {.callout-note}
JG: I wonder: Do we include the 2026 paper on reproducibility? https://www.nature.com/articles/s41586-026-10203-5. In a larger study (SCORE project) looiking at 600 papers from 2009-2018 about ~53%  of papers were able to be reproduced exactly. 
:::
...
interpreter itself or the system-level libraries those packages depend on (see
@tbl-glossary). Workflow orchestration tools such as {targets} [@landau_2021] and Make
...
an analysis proceeds, but they assume that the **software stack**[Is this in the library of defs?] required to run
...
chunks in `.Rmd` or `.qmd`—can be executed reliably. We illustrate these ideas 
...
::: {.callout-note}
JG: Some more defs are needed
- What is a stack?
- Literate programming?
- A terminal
- Nix
- @bruno some other ones?
:::
| Term | Definition |
|:-----------------------------|:--------------------------------------------------|
...
reported in the article. What might prevent them from obtaining identical 
...
`lavaan:::lav_graph_get_ancestors`), or to subtly different results 
...
the {rvinecopulib} package interfaces with a C++ backend and links against 
external libraries such as Boost, Eigen, and RcppThread [@rvinecopulib].
...
data itself. Indeed, a simulation study regenerates its data throughout the 
multiple runs. Therefore, the random number generator is part of the 
computational environment: reproducing the data requires not only the same seed 
but the same generator algorithm (e.g., Mersenne-Twister) and R version. 
Moreover, reproducing every statistical estimate exactly can depend on lower-level
components still, such as the numerical libraries R uses for matrix computations
...
Finally, analyses embedded in a literate programming workflow (documents that
combine narrative text and executable code) add further dependencies. Rendering
...
functional programming language, a build tool, and a package manager wrapped in one. This
...
on any machine. Researchers state exactly which versions of their language and
packages they need, and Nix works out and locks down the underlying system-level
...
{renv} cannot address [@rodrigues_baumann_2025]. Empirical work has shown strong
rebuildability and reproducibility rates for historical nixpkgs snapshots
[@rodrigues_baumann_2026_polyglot; @malka2026docker], and binary caches, 
which often allow environments to materialize in seconds, make Nix practical 
for interactive research workflows [@rodrigues_baumann_2025].
...
including its integration with rstats-on-nix for dated, reproducible snapshots, 
see @rodrigues_baumann_2025.

::: {.callout-note}
JG: Didnt the reviewer want us to spell out these things instead of refering to documentation?
:::

We also note that, although Nix is capable of replacing tools like Docker 
for isolation or {renv} for package management, it does not require an 
all-or-nothing transition. Researchers can adopt it gradually and use it 
alongside familiar tooling. For instance, by building Docker images with Nix, 
converting existing {renv} lockfiles, or running {targets} pipelines within a 
Nix-defined environment [@rodrigues_baumann_2025]. This allows Nix to 
strengthen reproducibility while preserving established workflows. 
...
wants it reproducible from the outset. Rather than writing the analysis first
and reproducing it later, we set up the computational environment with {rix}
first, then create and run the scripts and manuscript inside it. We then show
how another researcher could reproduce it. 

::: {.callout-note}
Let's be more specific here? How about we say we are a researcher using a Linux or mac compouter and show the steps needed for that setup? 
:::
...
`Why-risk-it-when-you-can-rix-it`). There are no simulation scripts or
manuscript inside it yet. The only file we write 
by hand for now is `generate-env.R` (or similar), which uses the `rix()` 
...
::: {.callout-note}
JG: Images might work for these steps? Showing what the folder should look like at each step. Just a thought. 
:::

We start with the smallest possible environment: R version and an IDE (RStudio). 
We add packages as the simulation takes form and extend the
environment again for the manuscript later. This bare specification is shown in
@lst-rix-env-base (the complete version, which covers the simulation and the
manuscript, is on the GitHub repository,
<https://github.com/felipelfv/Why-risk-it-when-you-can-rix-it>, as `gen-env.R`):

::: {.callout-note}
JG: I think we should also include  a zenodo link maybe in abstract. I think it will be more perm? 
...
```{r}
#| lst-label: lst-rix-env-base
#| lst-cap: "Bare environment specification: R and RStudio, no packages yet"
#| eval: false
#| echo: true
...
running `rix()` (@lst-rix-env-base). The file is written to
...
our illustration thus far, no packages were added yet. 
...
scripts. For the sake of clarity, one does not need to use RStudio, but we 
illustrate it because most readers probably make use of an IDE. 
As the code takes shape, we discover which 
R packages it needs and add them to `generate-env.R`, rerunning `rix()` and 
rebuilding each time. In other words, following the same steps as described 
above. The specification grows from the bare environment (@lst-rix-env-base) to
one that covers the simulation (@lst-rix-env-sim):

\small

```{r}
#| lst-label: lst-rix-env-sim
#| lst-cap: "Simulation environment, adding the packages the scripts need"
#| eval: false
#| echo: true
...
::: {.callout-note}
JG: why run nix-build and then show them the packages needed later on? Why not add from earlier step and highlight where in the code the arguments are important? I think this should be way earlier.
:::

```{r}
#| lst-label: lst-rix-env-manuscript
#| lst-cap: "Complete environment specification, adding the manuscript tools"
#| eval: false
#| echo: true
#| code-line-numbers: "5-14,16,17-27"
...
gives the reasons for adding each package in `r_pkgs` and `tex_pkgs`.

Again, users may add more packages "on-the-go" as one usually is not aware of 
all the packages that will be needed in advance. However, it is crucial that, 
after editing `generate-env.R`, we rebuild with `nix-build` and re-enter with
`nix-shell` so the new packages become available.
...
the Nix environment, anyone can reproduce them. A researcher interested in
reproducing the simulation run and results, given access to the project folder,
already has its `default.nix` and so does not need the authoring steps (Steps 1
to 3). With Nix installed, they need only build and enter the environment as in
Step 4 (`nix-build`, then `nix-shell`) and run the scripts in order. 
The advantage of executing within `nix-shell` is that all dependencies 
(the R version, packages, and system tools)
match exactly those specified in `default.nix`. However, this approach does
...
leaving the dependencies between scripts implicit in the code rather than 
explicitly declared.

## Step 6: Developing and Making the Complete Manuscript Reproducible[^11]
...
{knitr}, and a set of LaTeX packages. We add these to `generate-env.R` and
rebuild, giving the complete specification (@lst-rix-env-manuscript):
...
The `system_pkgs` parameter lists tools beyond R packages, such as Git, Pandoc,
or, here, Quarto, which is a command-line tool rather than an R package. Not
every system-level dependency needs listing. The {rvinecopulib} package, used in
the simulation, links against C++ libraries such as Boost, Eigen, and
RcppThread, yet Nix pulls these in automatically as dependencies of the package.
Our manuscript also uses the `apaquarto` extension for APA formatting, stored in
the project's `_extensions/` directory [@rodrigues_baumann_2025] (see {rix}
...
available, and we render the manuscript from the terminal (@lst-quarto-render):
...
::: {.callout-note}
JG: The system level depend should be specified earlier on and re-ref here. I dont think we want to keep rebuilding this. Give the readers everything and just refrene the appropiate lines. Maybe highlight?
:::

::: {.callout-note}
JG: This is exact issue R2 mentioned. Maybe state these are all the packages we will use however it can be rebuilt if new packages or system dependencies are needed?
:::

This command executes all code chunks in the manuscript 
(i.e., loads the R packages, runs the data generation and estimation), 
calculates and incorporates results (e.g., @tbl-results) and figures (e.g., Figure 1),
and generates a formatted PDF following APA style guidelines via the 
`apaquarto` extension [@schneider_2024]. This extension is saved in the project 
directory already. To add it to a project of your own, run the following inside 
the Nix shell, where Quarto is provided by the environment and therefore 
needs no separate installation (@lst-apaquarto-install):
...
The final document (which can be saved as a .docx, .pdf, or .html) is saved directly in the project
folder[^12]. Because Quarto is installed as a system-level package in our Nix
...
you from your existing system by default (as mentioned in footnote 7). For R
...
reproducibility. The second concerns workflow orchestration and integration
...
Because R's default generator can change across R versions, by pinning the R
version {rix} keeps the generator and its algorithm fixed, so the same seed
reproduces the same draws across machines and over time.
