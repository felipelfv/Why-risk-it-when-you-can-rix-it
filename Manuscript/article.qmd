...
is reproducibility (i.e., the ability to reproduce the results of a study or 
studies based on provided data, code, materials, and software/hardware) 
...
Reproducing such a study therefore means reproducing the data generation,
...
the high end. Reaching that executable end is often treated as *full* reproducibility
[@peikert_brandmaier_2021; @peikert_etall_2021], and open
...
studies, which depend on complex codebases, versioned dependencies, and
...
system (@fig-environment) [@rodrigues_2023; @rodrigues_baumann_2026_polyglot]. We
define *computational environment reproducibility* as the ability to reconstruct
this entire set of software dependencies on any machine and at any future time,
such that executing the same code yields the same numerical results. Empirical
assessments show that current practice falls short of this ideal. Indeed, 
@siepe_etall_2024 report that, among simulation studies published in 
psychology in 2021 and 2022, nearly two-thirds provide 
...
#| fig-cap: "The computational environment as a set of nested layers, based on Peikert & Brandmaier (2021)."
#| echo: false
#| message: false
#| fig-width: 7
#| fig-height: 4
...
orchestration system or to prescribe a particular analytic structure.[^1] Instead,
...
[^1]: We leave workflow orchestration aside for two reasons. First, many tools
...
Unlike familiar tools such as `install.packages()` in R or `uv` in Python, which
typically manage only a single layer of the software stack, Nix handles language
versions, package versions, and system-level dependencies within a single
framework [@rodrigues_baumann_2025]. Rather than
...
Once Nix is installed[^2], there are two ways to access {rix}, depending on
whether R is already installed on your system. In this tutorial, we proceed as
if R was already installed[^3] (@lst-install-rix-cran):

[^2]: It is worth noting that {rix} can generate Nix expressions even without
...
[^3]: We, however, recommend uninstalling your local R and letting Nix manage
...
automatically load the Nix environment when the project directory is opened[^5].
...
[^5]: **`direnv`** is a lightweight utility that integrates with the user's
...
[^6]: Please note that activating an RStudio instance via Nix does **not**
...
## Reproducing the Complete Manuscript[^7]

[^7]: See the {rix} documentation for more:
...
folder[^8]. Because Quarto is installed as a system-level package in our Nix
...
[^8]: Although we use `apaquarto` in this example, many alternative manuscript
...
stricter isolation[^9] that also prevents access to other system programs not 
specified in `default.nix`, use the `--pure` flag (@lst-nix-shell-pure):

[^9]: For example, when preparing this manuscript without the `--pure` flag,
