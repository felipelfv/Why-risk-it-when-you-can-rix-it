...
We now proceed from the lens of a researcher that wants to make their simulation
study and findings reproducible. We then show how another researcher would be
able to reproduce such study.
...
-   **Linux and Windows (WSL2)**:
    <https://docs.ropensci.org/rix/articles/setting-up-linux-windows.html>
-   **macOS**:
...
if R was already installed[^7] (@lst-install-rix-cran):
...
`Why-risk-it-when-you-can-rix-it`), where we separate this `article.qmd` (in the 
Manuscript subfolder) from the `.R` files inside the Simulation subfolder, where
the files mentioned before on p.7 are included. This is simply done for 
organization. After that, we want to establish a reproducible environment 
by creating a script that will generate the environment specification. 
We recommend creating a file named `generate-env.R` (or similar) in this 
same project directory. This script will use the `rix()` function from the 
{rix} package to produce a `default.nix` file: a declarative specification 
that precisely defines all software dependencies required for the project.

In our case, we implement the following environment specification, 
which can be found on the GitHub repository 
(<https://github.com/felipelfv/Why-risk-it-when-you-can-rix-it>) as a file 
named `gen-env.R` (@lst-rix-env-manuscript):
...
rix(
  ...
  ), # the R packages you need
  system_pkgs = c("quarto"), # non-R system tools
  tex_pkgs = c("amsmath", "ninecolors", "apa7", "scalerel",
    ...
    "anyfontsize"
  ), # LaTeX packages for the manuscript
...
As usually happens, one does not know in advance all the packages needed. This 
is not a problem. Researchers may add the desired packages in the arguments 
above "on-the-go", but must always rebuild the Nix environment, as we will 
illustrate below. Moreover, note that we have more than just the R packages 
specified for the simulation scripts. This happens because we also included 
what is needed for the manuscript generation, not solely for the simulation 
code. In Appendix B, we mention more specifically the reasons for adding each 
package in `r_pkgs()` and `tex_pkgs()`. For now, we focus more on clarifying 
the different arguments for the `rix()` function.

### The Environment Generation Script

The `rix()` function[^8] constructs this specification through a series of
...
demonstrates computational reproducibility not merely of the simulation
...
<https://docs.ropensci.org/rix/articles/installing-system-tools.html>). The
...
running `rix()` in an R console (@lst-rix-env-manuscript). The file is written to
the folder given in `project_path`, so you can confirm it succeeded by checking
that this folder now contains `default.nix`. The resulting `default.nix` file 
serves as the complete environment specification and contains all information 
required to recreate the project in a fully reproducible manner.

## Step 3: Building and Using the Reproducible Environment

Once Step 2 is complete, we build the reproducible environment by navigating to
...
## Reproducing the Complete Manuscript[^11]
...
As previously mentioned, researchers may prefer or not know how to use literate 
programming, or embedding the simulation within a dynamic document may be 
impractical. In this case, one could still follow the same steps shown thus far 
focusing only on the `.R` files while still creating and benefiting from a 
reproducible computational environment. For example, `03_run_simulation.R` 
begins by loading the required packages and sourcing other scripts that are 
needed:
