...
changes in default settings or numerical
implementations (e.g., `stringsAsFactors` defaulting to FALSE as of R 4.0). 
...
R is already installed on your system.
...
If R is already installed, {rix} can be installed from CRAN or from GitHub for 
the development version (@lst-install-rix-cran):
...
If R is not installed, Nix can be used to drop directly into an R session with 
{rix} available. Running the command in @lst-install-rix-nix-released provides 
the CRAN version; the command in @lst-install-rix-nix-dev provides the 
development version:

::: {#lst-install-rix-nix-released}
\small
``` bash
felipelfv@Felipes-MacBook-Pro Why-risk-it-when-you-can-rix-it % nix-shell 
  -p R rPackages.rix
```
R session with the released version of {rix} \normalsize
:::

::: {#lst-install-rix-nix-dev}
\small
``` bash
felipelfv@Felipes-MacBook-Pro Why-risk-it-when-you-can-rix-it % nix-shell 
  --expr "$(curl -sl https://raw.githubusercontent.com/ropensci/rix/
      main/inst/extdata/default.nix)"
```
R session with the development version of {rix} \normalsize
:::

For the remainder of this tutorial, we assume R is already installed on the 
system. Readers who accessed {rix} via the temporary Nix shell should execute 
the subsequent R commands from within that shell until the 
project-specific environment is built.
