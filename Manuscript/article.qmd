...
(e.g., BLAS/LAPACK), which Nix likewise pins as mentioned later.[^5]

[^5]: We do not mean that this exact level of identity must be met. Reproducing
    the broader pattern of results is arguably enough, but bit-for-bit 
    reproducibility simply happens to be a by-product of what Nix makes 
    possible.
...
quick start below covers the common case; complete, up-to-date instructions for
each operating system are maintained in the {rix} documentation:
...
[... To add a small installation example...]
...
active (here, `[nix-shell:~/AMPPS/Why-risk-it-when-you-can-rix-it]$`).
