...
[@hardwicke2020; @montoya_anderson_2026]. In a simulation study this involves 
...
[^3]: We leave workflow orchestration aside for two reasons. First, many tools
    ...
    although formal orchestration may be necessary for full reproducibility, a 
    study whose scripts are well documented, whose dependencies between files 
    are explained, and whose environment is made stable arguably already sits 
    high on the reproducibility spectrum. Moreover, a stable computational 
    environment is what the other practices ultimately rest on, and it is what 
    we focus on here. For readers interested in articles with a larger focus on 
    the workflow, we suggest @peikert_etall_2021 and @vanlissa_etall_2021.
...
within this document as executable chunks in a single `.qmd` file. This entails 
that the simulation runs from start to finish, producing results and figures 
automatically. This approach would be impractical for many real-world simulation 
studies, which are often too computationally intensive. We return to this 
trade-off later in the tutorial as the solution is quite simple: one may simply 
run the simulation outside the `.qmd` file and load the results for the analyses 
in the same file.
...
(e.g., BLAS/LAPACK). With respect to the latter, we do not mean that this 
exact level of identity must be met. Reproducing the broader pattern of results 
is arguably enough. However, as we will mention, the potential solution proposed 
here increases the chances of such bit-for-bit reproducibility, consistent with 
empirical findings [@malka2026docker].
...
[@rodrigues_baumann_2026_polyglot], and binary caches, which often allow
environments to materialize in seconds, make Nix practical for interactive
research workflows [@rodrigues_baumann_2025].
...
unfamiliar to most researchers, we recommend using {rix} to lower this barrier. The {rix} package provides an R-native interface: a 
...
directly, as {rix} translates automatically [@rodrigues_baumann_2025].
...
installed and configured first (see the {rix} documentation linked above). On 
all systems, install Nix by running the Determinate Systems installer in a
terminal[^5]. To open one: on macOS, go to Applications → Utilities → Terminal.
...
`Why-risk-it-when-you-can-rix-it`), where we
have the `article.qmd` inside the Manuscript subfolder and the `.R` files inside 
the Simulation subfolder. After that, we establish a reproducible environment 
by creating a script that will generate the environment 
specification. We recommend creating a file named `generate-env.R` (or similar) 
in the project directory. This script will use the `rix()` function from the 
{rix} package to produce a `default.nix` file: a declarative 
...
above "on-the-go", but must always rebuild the Nix environment, 
as we will illustrate below. Moreover, 
note that we have more than just the R packages specified for the
...
that this folder now contains `default.nix`. The resulting
...
which you are running the Nix project, as highlighted before. 
From the study directory, run the following command (@lst-nix-build):
...
To add it to a project of your own, run the following inside the Nix shell,
where Quarto is provided by the environment and therefore needs no separate
