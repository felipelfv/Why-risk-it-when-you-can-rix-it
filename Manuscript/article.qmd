...
Reproducing such a study therefore entails reproducing the data generation process,
the analysis, and the summary of results. @montoya_anderson_2026 conceptually
...
Even error-free code depends on a hierarchy of software components, including the
programming language version, the packages used in the analysis, and the 
system-level libraries those packages rely on. We define these and related terms 
used before the tutorial section in @tbl-glossary. When these differ from 
the originals, code may
...
complete software context required for an analysis to run successfully. This entails the
programming language version, package versions, system-level libraries, and operating
system (@fig-environment) [@rodrigues_2023; @rodrigues_baumann_2026_polyglot].
...
numerical results[^1]. Empirical assessments show that current practice falls short of 
this ideal. Indeed, @siepe_etall_2024 report that, among the 100 simulation studies they
identified in psychology journals in 2021 and 2022, nearly two-thirds provide
no accompanying code, and among those that do, documentation of the
computational environment is rarely included. This gap is consequential, as
simulation studies inform methodological recommendations, meaning that
insufficient reproducibility may undermine confidence in those recommendations
...
nearly the entire environment into a single executable image. Yet their use requires 
...
repositories [@malka2026docker]. For a detailed comparison of these tools and 
...
software ecosystem designed to make software installation deterministic, and {rix}
...
orchestration system or to prescribe a particular analytic structure.[^3] Instead,
...
chunks in `.Rmd` or `.qmd` files—can be executed reliably. We illustrate these ideas 
through a reproducible simulation study conducted in
...
earlier versions (e.g., the native pipe `|>` introduced in R 4.1). More subtly, changes
to R’s random number generation across versions mean that identical code
...
can depend on lower-level
components still, such as the system-level libraries R uses for matrix computations
(e.g., BLAS/LAPACK). With respect to the latter, we do not claim that this 
...
enough. Regardless, the solution we propose increases the chances of reaching 
such a level of reproducibility, consistent with empirical findings 
[@malka2026docker].
...
on other machines. Researchers state exactly which versions of their language and
packages they need, and Nix works out the underlying system-level libraries
automatically.
...
{renv} cannot address [@rodrigues_baumann_2025]. Empirical studies of historical nixpkgs
snapshots report that old environments can still be rebuilt years later, and
largely reproduce their original results [@rodrigues_baumann_2026_polyglot; @malka2026docker].
Moreover, because prebuilt binaries can be downloaded from a shared binary cache instead of
compiled from scratch, these environments are often ready in seconds, which keeps Nix
...
versions available on a chosen day), see @rodrigues_baumann_2025 and @rodrigues_baumann_2026_polyglot.
...
study who wants it reproducible from the outset. Rather than writing the analysis 
first and reproducing it later, we set up the computational environment with {rix}
first, then create and run the scripts and manuscript inside it. We then show
how another researcher could reproduce it.
...
documentation: Linux and Windows (WSL2) (<https://docs.ropensci.org/rix/articles/setting-up-linux-windows.html>) and 
...
the simulation environment (@lst-rix-env-sim) with the literate-programming tools
it lacks[^11]: Quarto, {knitr}, and a set of LaTeX packages. We add these arguments to
`gen-env.R` and rebuild. @lst-rix-env-manuscript shows only what is added:
...
system-level dependency needs listing, though: when an R package itself relies on
compiled libraries, Nix installs them automatically. The {rvinecopulib} package,
for example, links against the C++ libraries Boost and Eigen, which we never
list. The `apaquarto` extension we use for APA formatting is a separate case: it
is a Quarto extension stored in the project's `_extensions/` directory rather
than a Nix package [@rodrigues_baumann_2025] (see the {rix} documentation for
more: <https://docs.ropensci.org/rix/articles/installing-system-tools.html>).
...
is a single repetition within a given condition. Storing the generator's state at
the start of each condition-repetition cell, rather than only the initial seed,
additionally allows any single cell to be reproduced in isolation, which is
convenient for debugging [@morris_etall_2019]. In R, the state lives in the
...
loop, {doRNG} derives one independent stream per iteration in advance, before any
work is dispatched. Each stream is bound to the iteration index rather than to a
worker, so iteration seven always draws from stream seven no matter which core
executes it. The number of cores (`ncore` in our script) changes only how
...
interpreter or system-level libraries, Nix manages all layers of the software stack
...
to abandon it to benefit from Nix, as the two can be combined by using Nix inside
...
computing clusters where Docker is standard but Nix may not be available. In that
case, the environment is built with Nix but shipped inside a Docker image, so it
can be run with Docker alone on machines where Nix is not installed (see
{rix} documentation:
