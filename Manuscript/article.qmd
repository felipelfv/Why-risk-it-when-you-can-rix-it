...
| **Binary cache** | A server that stores prebuilt packages so Nix can download a finished copy instead of compiling it from source. Because each package is identified by a hash of its inputs, a cached binary is guaranteed to match the requested environment, making rebuilds fast without sacrificing reproducibility. |
...
our discussion into a typical methods section, but readers can
...
snapshots report that old environments can still be rebuilt and reproduce their
original results years later [@rodrigues_baumann_2026_polyglot; @malka2026docker].
...
versions available on a chosen day), see @rodrigues_baumann_2025.
...
but doing so means editing `generate-env.R`, rerunning `rix()`, and rebuilding
`default.nix`. In other words, following the steps we are about to illustrate. 
The simulation specification is shown in @lst-rix-env-sim (the
...
`generate-env.R` and rerun `rix()`. It is crucial that, after editing 
`generate-env.R`, we
...
{knitr}, and a set of LaTeX packages. We add these arguments to `generate-env.R`
and rebuild. @lst-rix-env-manuscript shows only what is added. The complete
`rix()` call is the `gen-env.R` file in the GitHub repository:
...
the simulation, links against C++ libraries such as Boost and
Eigen, yet Nix pulls these in automatically as dependencies of the package.
...
with a single `quarto render` of `article.qmd` (@lst-quarto-render), which
