...
used before the tutorial section,
...
illustrate it because most readers probably make use of an IDE. Because we
declared the simulation packages up front (@lst-rix-env-sim), they are already
available in the shell, and the scripts run against exactly those versions. 
In practice, one is rarely aware of every package a project will need in
advance. Packages can therefore be added "on-the-go": as the code takes shape
and calls for another package, we add it to `r_pkgs` in `generate-env.R`, rerun
`rix()`, and rebuild. It is crucial that, after editing `generate-env.R`, we
...
`rix()` call is the `gen-env.R` file in the project repository:
