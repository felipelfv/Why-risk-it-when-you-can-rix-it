...
is reproducibility (i.e., the ability to reproduce the results of a study based 
...
self-sufficient [@epskamp_2019; @wiebels_moreau_2021; @ziemann_etall_2023]. Even
...
LaTeX), each with its own version and installation requirements. 

# Nix and {rix}: A Comprehensive Solution

A potential solution is Nix [@dolstra_etall_2004], a software ecosystem built so
that installing software gives the same result every time. Nix is at once a
functional programming language, a build tool, and a package manager. This
...
We start with the smallest possible environment: R version and an IDE. 
