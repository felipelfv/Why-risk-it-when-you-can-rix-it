...
all system libraries---at the time of this writing. This eliminates the
...
(Listing~\ref{lst-rix-env-manuscript}) or from the terminal using
\texttt{Rscript} (Listing~\ref{lst-rscript-generate}):
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{felipelfv@Felipes{-}MacBook{-}Pro}\NormalTok{ your{-}project{-}directory \% quarto use template }
...
At this point, it is worth noting that Nix shells do not fully isolate
...
a reproducible computational environment. The
\texttt{Simulation\_Scripts/} folder contains the five sequential
scripts that depend on one another. For example,
...
\FunctionTok{source}\NormalTok{(}\StringTok{"Simulation\_Scripts/02\_models.R"}\NormalTok{)}
\NormalTok{...}
\end{Highlighting}
\end{Shaded}
...
After following Steps I-III (without needing to define packages for the
dynamic document rendering in Step I), the simulation study may be
reproduced within the Nix shell (Listing~\ref{lst-run-simulation}) as
follows:
...
match exactly those specified in \texttt{default.nix}. In the same way,
we could proceed with \texttt{04\_performance\_metrics.R} which loads
the simulation results (in
\texttt{Simulation\_Scripts/sim\_results.rds}) and calculates the
performance metrics, and \texttt{05\_plots.R} uses those saved metrics
(in \texttt{Simulation\_Scripts/performance\_summary.rds}) to create the
plots. Note, however, that this approach relies on manually running
scripts in sequence. It ensures a reproducible environment but does not
formalize the workflow itself; dependencies between scripts remain
...
The GitHub repository includes a branch demonstrating \{rixpress\} for
this entire project.
