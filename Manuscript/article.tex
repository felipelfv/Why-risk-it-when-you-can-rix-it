...
(\citeproc{ref-hardwicke2020}{Hardwicke et al., 2020};
\citeproc{ref-montoya_anderson_2026}{Montoya \& Anderson, 2026}). In a
simulation study this involves more than reanalyzing a fixed dataset,
because the code also generates the data. Reproducing such a study
therefore entails reproducing the data generation, the analysis, and the
summary of results. Montoya and Anderson
...
recommendations, meaning that insufficient reproducibility undermines
...
structure.\footnote{We leave workflow orchestration aside for two
  ...
  necessary for full reproducibility, a study whose scripts are well
  documented, whose dependencies between files are explained, and whose
  environment is made stable arguably already sits high on the
  reproducibility spectrum. Moreover, a stable computational environment
  is what the other practices ultimately rest on, and it is what we
  focus on here. For readers interested in articles with a larger focus
  on the workflow, we suggest Peikert et al.
  (\citeproc{ref-peikert_etall_2021}{2021}) and van Lissa et al.
...
(\texttt{05\_plots.R}) \footnote{We note that, for the sake of
  ...
  the simulation runs from start to finish, producing results and
  figures automatically. This approach would be impractical for many
  real-world simulation studies, which are often too computationally
  intensive. We return to this trade-off later in the tutorial as the
  solution is quite simple: one may simply run the simulation outside
...
With respect to the latter, we do not mean that this exact level of
identity must be met. Reproducing the broader pattern of results is
arguably enough. However, as we will mention, the potential solution
proposed here increases the chances of such bit-for-bit reproducibility,
consistent with empirical findings (\citeproc{ref-malka2026docker}{Malka
et al., 2026}).
...
dynamic document generation) additional layers of software dependencies
arise. For example, rendering R Markdown (\texttt{.rmd}) or Quarto
documents (\texttt{.qmd}) requires both a document conversion tool
(e.g., Pandoc, which converts \texttt{.rmd} or \texttt{.qmd} files into
formats such as PDF or HTML) and a typesetting system such as a LaTeX
distribution or Typst. Each of these components introduces its own
versioning constraints and platform-specific installation requirements.
Taken together, these layers highlight that reproducibility depends not
only on code and data, but also on the broader computational environment
in which analyses are executed.
...
for historical nixpkgs snapshots
(\citeproc{ref-rodrigues_baumann_2026_polyglot}{Rodrigues \& Baumann,
2026}), and binary caches, which often allow environments to materialize
in seconds, make Nix practical for interactive research workflows
(\citeproc{ref-rodrigues_baumann_2025}{Rodrigues \& Baumann, 2025}).
...
language is unfamiliar to most researchers, we recommend using \{rix\}
...
\subsubsection{Installing Nix}\label{installing-nix}
...
documentation linked above). Linux and macOS require no such step.

On all systems, install Nix by running the Determinate Systems installer
in a terminal\footnote{A \emph{terminal} is a text-based way to give
  your computer instructions by typing a command and pressing Enter.
  Readers new to the command line may find the Software Carpentry ``Unix
  Shell'' lesson a useful primer
  (\url{https://swcarpentry.github.io/shell-novice/}).}. To open one: on
macOS, go to Applications → Utilities → Terminal; on Windows, launch
your WSL2 (Ubuntu) shell from the Start menu; on Linux, open your
distribution's terminal application. Then run:
...
After installation, open a new terminal and enable the
\texttt{rstats-on-nix} binary cache, which provides pre-built R packages
and speeds up environment builds. With this installer, the cache is
configured through \{rix\}'s \texttt{setup\_cachix()} helper rather than
the standalone \texttt{cachix} client (which does not work here). Start
a temporary R session with Nix and run it:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{nix{-}shell} \AttributeTok{{-}p}\NormalTok{ R rPackages.rix}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rix}\SpecialCharTok{::}\FunctionTok{setup\_cachix}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

You then add yourself as a trusted user with a short, system-specific
command; see the \{rix\} documentation linked above for that step and
any platform-specific details.

\subsubsection{Installing \{rix\}}\label{installing-rix}
...
\texttt{Why-risk-it-when-you-can-rix-it}), where we have the
\texttt{article.qmd} inside Manuscript subfolder and the \texttt{.R}
files inside the Simulation subfolder. After that, one needs to
establish a reproducible environment by creating a script that will
generate the environment specification. We recommend creating a file
named \texttt{generate-env.R} (or similar) in the project directory.
This script will use the \texttt{rix()} function from the \{rix\}
package to produce a \texttt{default.nix} file---a declarative
...
needed. This is not a problem. We just need to add the desired packages
in the arguments above and rebuild the Nix environment, as we will
illustrate below. Moreover, note that we have more than just the R
packages specified for the simulation scripts. This happens because we
also included what is needed for the manuscript generation, not solely
for the simulation code. In Appendix B, we mention more specifically the
reasons for adding each package in \texttt{r\_pkgs()} and
\texttt{tex\_pkgs()}. For now, we focus more on clarifying the different
arguments for the \texttt{rix()} function.
...
\url{https://docs.ropensci.org/rix/articles/installing-system-tools.html}).
The tex packages included in the above code offer a nice starting point
for researchers wanting to create manuscripts using LaTeX.
...
Once Step 2 is complete, build the reproducible environment by
...
From the study directory, run the following command
...
\url{https://docs.ropensci.org/rix/articles/binary-cache.html} for
instructions.
...
results (e.g., Table 1) and figures (e.g., Figure 1), and generates a
formatted PDF following APA style guidelines via the \texttt{apaquarto}
extension (\citeproc{ref-schneider_2024}{Schneider, 2024}). This
extension is saved in the project repo already. To add it to a project
of your own, run the following inside the Nix shell, where Quarto is
provided by the environment and therefore needs no separate installation
...
As previously mentioned, researchers may prefer not to use literate
programming, or embedding the simulation within a dynamic document may
be impractical. In this case, one could still follow the same steps
shown thus far focusing only on the .R files while still benefiting from
a reproducible computational environment. For example,
\texttt{03\_run\_simulation.R} begins by loading the required packages
and sourcing other scripts that are needed:
...
\FunctionTok{source}\NormalTok{(}\StringTok{"Simulation\_Scripts/01\_data\_generation.R"}\NormalTok{)                    }
\NormalTok{...}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\ExtensionTok{[nix{-}shell:\textasciitilde{}/Why{-}risk{-}it{-}when{-}you{-}can{-}rix{-}it]$}
  \ExtensionTok{Rscript}\NormalTok{ Simulation\_Scripts/04\_performance\_metrics.R}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\ExtensionTok{[nix{-}shell:\textasciitilde{}/Why{-}risk{-}it{-}when{-}you{-}can{-}rix{-}it]$}
  \ExtensionTok{Rscript}\NormalTok{ Simulation\_Scripts/05\_plots.R}
\end{Highlighting}
\end{Shaded}
