...
It is worth noting that Nix shells do not fully isolate you from your
existing system by default. For R users, this has a practical
implication: packages installed in your regular R library (outside of
Nix) could potentially be loaded when running R from within the Nix
environment. The \{rix\} package addresses this automatically---when you
call \texttt{rix()}, it also executes \texttt{rix\_init()}, which
creates a project-specific \texttt{.Rprofile}. This file configures R to
ignore external package libraries and also disables
...
environment. The \texttt{Simulation\_Scripts/} folder contains the five
sequential scripts that depend on one another through \texttt{source()}
calls. For example, \texttt{03\_run\_simulation.R} begins by loading the
required packages and sourcing other scripts that are needed:
...
To then run the simulation within the nix shell, after following Steps
I-III (without needing to define packages for the dynamic document
rendering in Step I), the simulation study may be reproduced
...
that all dependencies---R version, packages, and system tools---match
exactly those specified in \texttt{default.nix}. In the same way, we
could proceed with \texttt{04\_performance\_metrics.R} which loads the
simulation results and calculates the performance metrics, and
\texttt{05\_plots.R} uses those metrics to create the plots.

\subsection{Additional Considerations for Advanced
Workflows}\label{additional-considerations-for-advanced-workflows}

As highlighted here and previous literature
(\citeproc{ref-peikert_brandmaier_2021}{Peikert \& Brandmaier, 2021};
\citeproc{ref-piccolo_frampton_2016}{Piccolo \& Frampton, 2016};
\citeproc{ref-siepe_etall_2024}{Siepe et al., 2024}), simulation studies
(and any analysis in general) should in principle also have the
computational workflow reported completely. This may achieved through
different ways, such as Make files, targets in a R-focused scenario, but
in this section we provide a brief illustration with \{rixpress\}. Note
we do not show how to use it as that would entail a tailored and large
section in this tutorial. Moreover, we also provide additional
considerations about certain tools aforementioned.
...
For the interested reader, in the GitHub repository we included a second
branch that uses \{rixpress\} for this entire project, in which the
simulation and manuscript are generated.
...
\bottomrule\noalign{}
\endlastfoot
\textbf{rvinecopulib} & \texttt{01\_data\_generation.R} & Generates
correlated data via copulas (\texttt{rbicop()}) \\
\textbf{marginaleffects} & \texttt{02\_models.R} & Computes average
causal effects via \texttt{avg\_slopes()} \\
\textbf{doParallel} & \texttt{03\_run\_simulation.R} & Enables parallel
\texttt{foreach} loops across CPU cores \\
\textbf{doRNG} & \texttt{03\_run\_simulation.R} & Makes parallel RNG
reproducible \\
\textbf{simhelpers} & \texttt{04\_performance\_metrics.R} & Calculates
bias, RMSE, coverage metrics \\
\textbf{ggplot2} & \texttt{05\_plots.R} & Creates simulation result
visualizations \\
\textbf{cowplot} & \texttt{05\_plots.R} & Combines plots with
\texttt{plot\_grid()}, extracts legends \\
\textbf{dplyr} & \texttt{article.qmd} & Data wrangling for results
...
\bottomrule\noalign{}
\endlastfoot
\textbf{environ} & tcolorbox \\
\textbf{pdfcol} & tcolorbox \\
\textbf{tikzfill} & tcolorbox \\
\textbf{fontaxes} & newtx \\
\textbf{xstring} & Template conditionals \\
\textbf{scalerel} & apa7 class \\
\end{longtable}
...
\textbf{endfloat} & Moves floats to end of document in manuscript
mode \\
...
\textbf{fvextra} & Enhanced verbatim for syntax-highlighted code
blocks \\
