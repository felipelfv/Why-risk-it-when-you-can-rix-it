...
recent stable release. Alternatively, the `date` argument specifies a
...
the R ecosystem at a single point in time. For this tutorial, as shown on top, 
we use the `date` parameter to ensure temporal 
...
The `r_pkgs` argument accepts a character vector listing all required R
...
argument accepts a list structure containing repository URLs and specific
commit hashes. For example:
...
error—Quarto's error messages during, for example, the PDF rendering 
indicate which packages are missing, and these can then be 
added to `tex_pkgs` (see {rix} documentation for more:
<https://docs.ropensci.org/rix/articles/d2-installing-system-tools-and-texlive-packages-in-a-nix-environment.html>).
...
project-specific version of RStudio within the Nix environment. This is
required for RStudio because, unlike other editors, RStudio cannot interact with
Nix shells unless it is itself installed via Nix. Note that on macOS, RStudio is
...
These editors can either be installed within the Nix environment using the 
`ide` parameter, or researchers can use an already-installed version by 
setting `ide = "none"` (or `ide = "other"`) and configuring 
`direnv` to automatically load the Nix environment when opening the project 
folder. Each IDE installed via Nix is project-specific and will not interfere 
with system-wide installations. See the
...
stricter isolation[^8] that also prevents access to other system 
...
the build failed. Running `quarto check` from within the Nix shell 
(i.e., `nix-shell --run "quarto check"`) revealed that Quarto was still 
accessing the system's LaTeX installation (`/Library/TeX/texbin`) rather 
...
simulation and calculates the results as well as visualizations. 
The script begins by loading the required packages, then proceeds 
through three steps: first, it sources `03_run_simulation.R`, which in turn 
loads the data generation function (`01_data_generation.R`) and the model 
estimation functions (`02_models.R`) to run the Monte Carlo simulation; second, 
it sources `04_performance_metrics.R` to calculate the performance criteria 
from the saved simulation results; and third, it sources `05_plots.R` to 
generate the figures (see @fig-performance). The script has the following 
content (@lst-run-all):
...
(i.e., `Rscript 03_run_simulation.R)`). Therefore, the key advantage of 
executing within `nix-shell` is that all 
...
manuscript as well as the reporting (i.e., performance metrics calculation and 
visualization). In other words, the code in the separate .R files are included 
as code chunks (see `article.qmd` in the GitHub repository). 
...
completed. Thus, in this case, one could just proceed with the 
`Rscript 03_run_simulation.R` inside the nix shell. Then, in the `article.qmd` 
load the obtained results and integrate the code chunks that produce 
the performance metrics and visualization (see Figure 1). 
