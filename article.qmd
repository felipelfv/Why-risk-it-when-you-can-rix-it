...
next subsection (i.e., Specifying the Computational Environment; in @lst-rix-env).
...
(again, see @lst-rix-env in the next subsection). Alternatively, if you prefer 
...
shown, which can be found on the GitHub repository as a 
file named `generate_env.R` (@lst-rix-env):
...
Note that we have more than the packages specified for the simulation scripts.
In Appendix B, we mention more specifically the reasons for adding each package 
in `r_pkgs()` and `tex_pkgs()`. 
...
`Rscript generate_env.R`, sourcing it from within R using 
`source("generate_env.R")`, or running the `rix::rix()` function above, 
generates the `default.nix` file in the project directory. 
This file serves as the formal, machine-readable
...
library. However, this will only work for R version 4.4.3 at least, or for a 
date on or after the 2025-02-28 (see <https://docs.ropensci.org/rix/articles/e-configuring-ide.html>).
