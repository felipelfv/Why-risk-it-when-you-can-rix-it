...
calculating simulation performance metrics [@cite]. 
Parallel processing is handled by {doParallel} [@cite], with {doRNG} [@cite] 
ensuring reproducible random number generation across parallel workers. 
Visualization relies on {ggplot2} [@cite]. 
...
this version sensitivity is consequential. If one also employs literate 
programming, additional dependencies arise: R Markdown or Quarto documents 
...
[^1]: Note that the rationale, programming-related choices, and results should not be used for substantial interpretation. For instance, the usage of {rvinecopulib} and the parallel set-up may be considered an overhead.
...
**Installing on Linux and Windows**[^6].Once your Linux or WSL2 environment
is ready, install Nix using the Determinate Systems installer: 

[^6]: Windows users who choose not to enable `systemd` may need to append `--init none`; see {rix} documentation highlighted above.
...
**Installing on macOS**. On macOS, install Nix using the same Determinate Systems installer:
...
### The Environment Generation Script[^7]
