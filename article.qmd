...
R (Example 1), culminating in an automated APA-formatted manuscript generated with 
apaquarto [@schneider_2024] (Example 2). Although the example centers on R because of
...
readers to the Appendix A, which presents a simulation study scenario designed to 
...
The data generation depends on the {rvinecopulib} package [@rvinecopulib]. 
The {marginaleffects} provides the function for computing 
average causal effects [@marginaleffects]. The {dplyr} package is used for data 
wrangling [@dplyr] and {simhelpers} supplies the functions for calculating the
simulation performance metrics [@simhelpers]. Parallel processing is handled by 
{doParallel} [@doParallel], with {doRNG} [@doRNG] ensuring reproducible random 
number generation across parallel workers. Visualization relies on {ggplot2} 
[@ggplot2] and {cowplot} [@cowplot]. 
...
lockfiles, or running {targets} pipelines within a Nix-defined environment [@rodrigues_baumann_2025]. This allows Nix to strengthen 
reproducibility while preserving established workflows.
...
To enable WSL2, run the following in PowerShell as an administrator (@lst-enable-wsl): 
...
projects, which you then build with Nix. This latter part is explained in the next subsection (i.e., [Specifying the Computational Environment](#sec-specifying-environment)).
...
(again, see [Specifying the Computational Environment](#sec-specifying-environment)). Alternatively, if you prefer 
the stable CRAN version of {rix}, you can create a 
temporary shell with (@lst-nix-shell-cran): 
...
rix(
  ...
  ide = "", # line 3
  project_path = ".", # line 4
  overwrite = TRUE # line 5
)
```
\normalsize

In the case where we use literate programming for generating the manuscript
(Example 2), we implement the environment specification 
shown below, which can be found on the GitHub repository as a 
...
scripts. This happens because we also included what is needed for 
the manuscript generation, not solely for the simulation code. 
In Appendix B, we mention more specifically the reasons for adding each package 
in `r_pkgs()` and `tex_pkgs()`. For now, we focus more on clarifying the 
different arguments for the `rix()` function. 
...
commit hashes [@rodrigues_baumann_2025]. This ensures that exact development 
versions are obtained. For our simulation study, all packages were used with 
their CRAN versions (see {rix} documentation for more:
<https://docs.ropensci.org/rix/articles/d2-installing-system-tools-and-texlive-packages-in-a-nix-environment.html>).
...
error—Quarto's error messages during PDF rendering indicate which packages are
missing, and these can then be added to `tex_pkgs` (see {rix} documentation for
more:
<https://docs.ropensci.org/rix/articles/d2-installing-system-tools-and-texlive-packages-in-a-nix-environment.html>).
...
which is another argument, displays the generated specification in 
the console for immediate verification [@rodrigues_baumann_2025].
...
packages 
(e.g., `py_conf = list(py_version = "3.12", py_pkgs = c("polars","pandas"))`). 
Similarly, `jl_conf` enables Julia package installation. This
...
Python or numerical optimization in Julia [@rodrigues_baumann_2025] 
(see {rix} documentation for more:
...
`Rscript generate_env.R`, source it from within R using 
...
As mentioned, it is worth noting that Nix shells do not fully 
isolate you from your existing 
system by default. For R users, this has a practical implication: packages 
installed in your regular R library (outside of Nix) could potentially be 
loaded when running R from within the Nix environment. The {rix} package 
addresses this automatically—when you call `rix()`, it also executes 
`rix_init()`, which creates a project-specific `.Rprofile`. This file 
...
preserving full reproducibility [@rodrigues_baumann_2025].

However, for stricter isolation[^8] that also prevents access to other system 
programs not specified in `default.nix`, use the `--pure` flag 
(@lst-nix-shell-pure):

[^8]: For example, when preparing this manuscript without the `--pure` flag, 
running `quarto check` 
(i.e., `felipelfv@Felipes-MacBook-Pro Why-risk-it-when-you-can-rix-it % nix-shell --run "quarto check"`) from within the Nix shell revealed that Quarto was 
still accessing the system's LaTeX installation (`/Library/TeX/texbin`) rather 
than being restricted to only what was specified in `default.nix`.
...
# estimation function and another function for calculate the true ACE based 
# on a large sample size
source("03_run_simulation.R")

# step 2: calculate performance metrics
# this file calculates the performance criteria based on the saved results 
# from the 03_run_simulation.R
...
performance metrics and saving the calculated values to 
`performance_summary.rds`, and finally generating all figures. 
Alternatively, individual scripts can be executed separately. 
The key advantage of executing within `nix-shell` 
is that all dependencies—R version, packages, and system tools—match exactly 
those specified in `default.nix`.
...
integrate the code chunks that produce the plots (see Figure 1). To render the 
manuscript (@lst-quarto-render): 
