...
# Load required packages for display/tables
library(ggplot2)
library(cowplot)
library(knitr)
library(dplyr)
library(rixpress)
# Load pre-computed results from rixpress pipeline
rixpress::rxp_load("performance_summary")
rixpress::rxp_load("sim_results")
...
badges [@kidwell_etall_2016] and platforms like the Open Science Framework have
created "challenges" to make data and code sharing increasingly
routine.[@levenstein_lyle_2018]. However, these efforts largely occupy the lower
and middle portions of the continuum, emphasizing what is shared rather than how
shared materials can be executed in practice.

Data and code are never fully self-sufficient to reproduce a set of findings.
Assuming the data and code are complete and accurate, reproducibility depends on
a hierarchy of software components—collectively referred to as
...
data-generating conditions (see Appendix A for full technical details). After
completing the analysis and publishing the results, another researcher wishes to
reproduce your findings—running the same code and obtaining the same numerical
results.

This tutorial is organized around that scenario. We use a stylized simulation
...
software dependencies arise. For example, rendering R Markdown or Quarto
documents requires both a document conversion tool (e.g., Pandoc, which converts
`.Rmd` or `.qmd` files into formats such as PDF or HTML) and a typesetting
system such as a LaTeX distribution or Typst. Each of these components
introduces its own versioning constraints and platform-specific installation
requirements. Taken together, these layers highlight that reproducibility
depends not only on code and data, but also on the broader computational
environment in which analyses are executed.
...
isolated across platforms. In practical terms, this means that Nix allows
researchers to specify *exactly* which versions of programming languages,
packages, and system libraries an analysis requires, and to recreate that same
environment reliably on another machine.
...
# Data generation function - defined in functions/gen_data.R
# Results are pre-computed by the rixpress pipeline
...
# Model estimation functions - defined in functions/run_one_rep.R and functions/compute_true_ace.R
# Results are pre-computed by the rixpress pipeline
...
# Simulation loop - executed via rixpress pipeline with 9 parallel condition derivations
# sim_results loaded via rxp_load() at top of document
...
# Performance metrics - computed via rixpress pipeline
# performance_summary loaded via rxp_load() at top of document
...
As described before, \[...\] For streamlined execution, we provide a master
script (`06_run_all.R`) that runs the complete simulation and calculates the
results as well as visualizations. The script begins by loading the required
packages, then proceeds through three steps: first, it sources
`03_run_simulation.R`, which in turn loads the data generation function
(`01_data_generation.R`) and the model estimation functions (`02_models.R`) to
run the Monte Carlo simulation; second, it sources `04_performance_metrics.R` to
calculate the performance criteria from the saved simulation results; and third,
it sources `05_plots.R` to generate the figures (see @fig-performance). The
script has the following content (@lst-run-all):
